...
  const FEEDBACK_TOGGLE_KEY = "isFeedbackWindowEnabled"; // New persistence key
...
    } catch (e) {
      console.warn("Firebase loading failed. Feedback data cannot be persisted.", e);
    }
  }
...
      
      /* Warnings & Toast (UPDATED position and styling) */
...
  // FIRESTORE LOGIC
  // -----------------------
  // Note: Firestore logic is kept for compatibility with the original script,
  // but the new toggle feature uses localStorage for client-side preference.
...
    const db = window._survey_db;
    if (!db) return;
    try { await db.collection(SURVEY_RATE_COLLECTION).doc(uid).set(data, { merge: true }); } catch (e) {}
  }
...
    const submitBtn = el("button", { className: "open-survey-btn", innerText: "Submit" }); // Changed from "Send Feedback" to "Submit"
    const laterBtn = el("button", { className: "close-sidebar-btn", innerText: "Close", style: "margin-top:12px;" }); // Changed from "Cancel" to "Close"
    
...
    laterBtn.onclick = () => popup.remove();
    ...
    // NEW: Stop Feedback Button Handler
...
    const t = el("div", { id: "survey-toast" });
    t.style.bottom = '24px';
    t.style.right = '24px';
...
    if (!userAcceptedCookies()) return;
    // NEW: Check if the window is enabled by the user
    if (!getFeedbackState()) return;

    setTimeout(async () => {
      try { await ensureFirebase(); } catch(e){}
      // Guard against Firebase failure if logic depends on it
      if (!window._survey_db || !window._survey_auth) {
...
      const check = await canTakeSurvey(uid).catch(()=>({ok:true}));
      ...
      // Re-check state before showing, in case Alt+W was pressed during async wait
      if (!getFeedbackState()) return;
